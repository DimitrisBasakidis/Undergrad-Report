...
Upon completion of each control interval, FlexHeap aggregates the lost compute
cycles from GC and I/O and passes them to a finite-state machine (FSM). The FSM
acts as a control mechanism that reacts to relative (percent) changes in GC and
I/O overhead and decides whether to adjust the DRAM allocation between the
main heap (H1) and the page cache for H2. The FSM consists of three states. The
three states are shown in Fig.~\ref{fig:flexheap-fsm}, and we analyze them next.
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.9\linewidth]{fig/fsm.png}
  \caption{FlexHeap control FSM: states and conditions for state transitioning.}
  \label{fig:flexheap-fsm}
\end{figure}
