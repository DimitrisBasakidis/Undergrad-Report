\begin{abstract}
  ...
  over a storage device and is accessed via the OS page cache.
  The partition of memory between H1 and the page-cache for H2, has to be
	statically configured at compile time, not enabling system adaptability during execution. 
  H1 may need more memory, when
	...
	In this thesis, we address the DRAM division problem in TeraHeap’s static DRAM
	split, by integrating  FlexHeap, a dynamic heap resizing mechanism.
	FlexHeap is, a policy that monitors at runtime GC and I/O.
	...
	average by 28\% and 13\%. Similarly, it reduces the total runtime in the majority
  of Spark's benchmarks, by 6\% on average.
